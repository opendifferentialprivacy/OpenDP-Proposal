...
def main():
    ...
    identity = opendp.ops.make_identity(str_to_c_char_p("<String>"))
    ...
    split_dataframe = opendp.ops.make_split_dataframe(str_to_c_char_p(","), 3)
    parse_column_1 = opendp.ops.make_parse_column(str_to_c_char_p("<i32>"), split_dataframe, str_to_c_char_p("1"), True)
    parse_column_2 = opendp.ops.make_parse_column(str_to_c_char_p("<f64>"), parse_column_1, str_to_c_char_p("2"), True)
    parse_dataframe = opendp.core.make_chain(parse_column_2, opendp.core.make_chain(parse_column_1, split_dataframe))
    arg = opendp.data.from_string(str_to_c_char_p("ant, 1, 1.1\nbat, 2, 2.2\ncat, 3, 3.3"))
    ret = opendp.core.operation_invoke(parse_dataframe, arg)
    dump(opendp, ret)
    opendp.data.data_free(arg)
    opendp.data.data_free(ret)
    opendp.core.operation_free(parse_dataframe)
