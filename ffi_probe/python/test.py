...
def main():
    ...
    ret = odp.core.operation_invoke(identity, arg)
    print(odp.to_str(ret))
    odp.data.data_free(arg)
    odp.data.data_free(ret)
    odp.core.operation_free(identity)
    ...
    parse_dataframe = odp.make_chain_multi(parse_column_2, parse_column_1, split_dataframe)
    ...
    noisy_sum_1 = odp.make_chain_multi(base_laplace_1, bounded_sum_1, clamp_1, select_1)
    ...
    noisy_sum_2 = odp.make_chain_multi(base_laplace_2, bounded_sum_2, clamp_2, select_2)

    # Compose & chain
    composition = odp.core.make_composition(noisy_sum_1, noisy_sum_2)
    everything = odp.make_chain_multi(composition, parse_dataframe)

    # Do it!!!
    arg = odp.data.from_string(b"ant, 1, 1.1\nbat, 2, 2.2\ncat, 3, 3.3")
    ret = odp.core.operation_invoke(everything, arg)
    print(odp.to_str(ret))

    # Clean up
    odp.data.data_free(arg)
    odp.data.data_free(ret)
    odp.core.operation_free(everything)
